...
window.Stacketry = (function(window, document, undef) {
    ...
    function _bootstrapAngular(runtime) {
      console.log('Bootstrapping Angular applicaiton "' + runtime.ngApp + '"')
      ...
      runtime.routes.forEach(function(route) {
        ...
        // load each route's requires via it's manifest
        var manifestUrl = _join(sitePath, route, 'manifest.json')
        _fetch(manifestUrl).then(
          function(data) {
            console.log('here')
            // parse save manifest
            manifest = JSON.parse(data)
            _loadRouteRequires(route, manifest ).always(_continue)
          },
          function(status) {
            // supply default manifest
            var defaultManifest = {
              "name": route,
              "templateUrl": _join(sitePath, route, 'index.html'),
              require: [
                { 
                  "path" : _join(sitePath, route, 'index.css'),
                  "type" : "text/css" 
                },
                { 
                  "path" : _join(sitePath, route, 'index.js'),
                  "type" : "text/javascript"
                }
              ]
            }
            console.log('using default manifest for route', route)
            _loadRouteRequires(route, defaultManifest ).always(_continue)
          }
        ) // _fetch()

      }) //forEach 

      // next, bootstrap angular 
    ...
    /**
      ...
      */
    function _loadRouteRequires(route, manifest) {
      var deferred = new Promises.Deferred()

      // TODO: LOOP!!!!!! through manifest.require and load everything!
      ...
      yepnope.injectCss(manifest.require[0].path)  

     
      // Load route specific js
      _fetch(manifest.require[1].path).then(
        function(result) {
          var exports = injectScript(result.data)
          // save these exports?
        },
        function(status) {
          console.log('error loading scirpt', manifest.require[1].path)
        }
      )
    
      //TODO: Should this wiat for sciprts to load????
    ...
    function injectScript(script) {    
      ...
      try {
        ...
        console.log("Stacketry: External Script Exception: \n", e)
      }
